...
{
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <ped file> <db name> <query>\n",argv[0]);
        return 1;
    }

    char *ped_file_name = argv[1];
    char *db_name = argv[2];
    char *select_query = argv[3];
    ...
    char *test_q;
    r = asprintf(&test_q, "SELECT Ind_ID FROM ped WHERE %s;", select_query);
    ...
    uint32_t *ids = (uint32_t *) malloc(ll.len * sizeof(uint32_t));
